/* wrtlst.f -- translated by f2c (version 20100827).
...
/* Determine low temperature opacities flag, LOK */
    s_copy(lok, "NONE", (ftnlen)4, (ftnlen)4);
    if (newopac_1.lalex95) {
	s_copy(lok, "ALEX", (ftnlen)4, (ftnlen)4);
    }
    if (newopac_1.lkur90) {
	s_copy(lok, "KURZ", (ftnlen)4, (ftnlen)4);
    }
/* Determine high temperature opacities flag, HIK */
    ...
    putmodel2_(bl, &cfenv[1], &const3_1.cmixl, dage, ddage, ftri, &hcomp[16], 
	    ...
	    lok, &dpmix_1.lovstc, &dpmix_1.lovste, &dpmix_1.lovstm, &
