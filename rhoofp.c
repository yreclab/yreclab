/* rhoofp.f -- translated by f2c (version 20100827).
...
/* ******************************************************************** */
doublereal rhoofp_(doublereal *x, doublereal *ztab, doublereal *t6, 
...
/*      IF(IRAD .EQ. 1) PR=4.D0/3.D0*RAT*T6**4   ! MB */
	...
	rdbg = .001;
	idbg = 1;
	switch (esac_(&xdbg, ztab, &t6dbg, &rdbg, &idbg, irad)) {
	    case 1:  goto L999;
	}
    }
    ilo = 2;
...
/*      WRITE(ISHORT,'("PNR, PMAX,PMIN=",3E14.4)') PNR,PMAX,PMIN */
    ...
    switch (esac_(x, ztab, t6, &rhog1, &c__1, irad)) {
	case 1:  goto L999;
    }
    p1 = e_1.eos[0];
    if (p1 > pnr) {
...
/* Had wrong pointer, see RHOG1= ten lines up */
	switch (esac_(x, ztab, t6, &rhog2, &c__1, irad)) {
	    case 1:  goto L999;
	}
	p2 = e_1.eos[0];
    }
...
L1:
    ...
    switch (esac_(x, ztab, t6, &rhog3, &c__1, irad)) {
	case 1:  goto L999;
    }
    p3 = e_1.eos[0];
