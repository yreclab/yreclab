/* getnewenv.f -- translated by f2c (version 20100827).
...
/* Subroutine */ int getnewenv_(doublereal *senvold, doublereal *hcomp, 
	...
	doublereal *ht, logical *lc, doublereal *eta2, doublereal *hg, 
	doublereal *hi, doublereal *hjm, doublereal *qiw, doublereal *r0, 
	doublereal *hkerot, doublereal *bl, doublereal *sjtot, doublereal *
	skerot, doublereal *teffl, integer *m, logical *lnew)
{
    /* Format strings */
    ...
    static doublereal omega[5000], sumke, hsbase;
    ...
    /* Fortran I/O blocks */
    static cilist io___48 = { 0, 6, 0, fmt_910, 0 };
    static cilist io___49 = { 0, 6, 0, fmt_911, 0 };
    static cilist io___50 = { 0, 6, 0, fmt_597, 0 };
...
/* MHP 10/02  define ISTORE - used in ENVINT */
    ...
    envint_(&b, &fpl, &ftl, &gl, hstot, &ixx, &lprt, &lsbc0, &plim, &rl, 
	    teffl, &x, &z__, dum1, &idum, &katm, &kenv, &ksaha, dum2, dum3, 
	    dum4, &lpulpt);
...
/* MASS CONTENTS OF THE NEW SHELLS (HS2). */
    ...
    hs2[*m] = pow_dd(&c_b6, hstot) - hsbase;
...
/* CONSTANT J/M */
		...
		hjm[j] = hjm[mm];
		d__1 = hr[j] * 2.;
		hi[j] = const1_1.cc23 * pow_dd(&c_b6, &d__1);
...
/* GENERAL CASE */
	    ...
	    w0 = omega[mm - 1] * pow_dd(&c_b6, &d__1);
	    i__1 = *m;
	    for (j = mm + 1; j <= i__1; ++j) {
		d__1 = hr[j] * rot_1.walpcz;
		omega[j - 1] = w0 / pow_dd(&c_b6, &d__1);
		d__1 = hr[j] * 2.;
		hi[j] = const1_1.cc23 * pow_dd(&c_b6, &d__1);
