/* massloss.f -- translated by f2c (version 20100827).
	...
	doublereal *senvold, logical *lnewfit)
{
    ...
    static doublereal p, r__, t, v, x, z__, ethaccbar, dl, gl, dr, pl, rl, tl,
	     eta, eth, amu, qcp, qdp, qat, emu, qdt, qap, rmu, eacc, dela, 
	    beta, dmdt, qcpp;
...
/* CONVECTIVE OVERTURN TIMESCALE */
	...
	for (i__ = *jenv + 1; i__ <= i__1; ++i__) {
	    v = (scrtch_1.svel[i__ - 2] + scrtch_1.svel[i__ - 1]) * .5;
	    dr = pow_dd(&c_b3, &hr[i__]) - pow_dd(&c_b3, &hr[i__ - 1]);
	    if (v > 0.) {
		deuter_1.taucz += dr / v;
	    }
	}
	s_wsle(&io___12);
	d__1 = deuter_1.taucz / const3_1.csecyr;
