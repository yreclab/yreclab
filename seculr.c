/* seculr.f -- translated by f2c (version 20100827).
...
/* $$$$$$ */
/* Subroutine */ int seculr_(doublereal *delts, doublereal *dt, doublereal *
	hd, doublereal *hg, doublereal *hi, doublereal *hl, doublereal *hp, 
	doublereal *hr, doublereal *hs, doublereal *hs1, doublereal *hs2, 
	doublereal *ht, doublereal *hjmsav, logical *lcz, logical *lc, 
	integer *m, integer *model, doublereal *omega, doublereal *hjm, 
	doublereal *eta2, doublereal *qiw, doublereal *r0, doublereal *hcomp, 
	logical *lfirst, integer *imin, integer *imax, doublereal *bl, 
	doublereal *hstot, doublereal *sjtot, doublereal *smass, doublereal *
	teffl, logical *lredo, integer *iredo, doublereal *hicz, doublereal *
	hsbot, doublereal *hstop, doublereal *omegas, logical *lczsur, 
	integer *mrzone, integer *mxzone, integer *nrzone, integer *nzone, 
	doublereal *cod2, doublereal *hv, logical *lok)
{
    /* Format strings */
    ...
    static doublereal em[5000], ei[5000], ej[5000], dj[5000], ew[5000];
    static integer it;
    static doublereal dr, dx, dr1, dc12, dc13, cod[5000], dn14;
    static logical ldo;
    static integer idm;
    ...
    static logical lokad;
    extern /* Subroutine */ int vcirc_(doublereal *, doublereal *, doublereal 
    ...
    extern /* Subroutine */ int eq2mod_(doublereal *, doublereal *, 
	    ...
	    *), checkj_(doublereal *, doublereal *, doublereal *, doublereal *
	    ...
	    logical *, doublereal *, doublereal *, doublereal *, doublereal *,
	     integer *, integer *, logical *), codiff_(doublereal *, 
	    doublereal *, doublereal *, integer *, integer *, logical *, 
	    logical *, integer *, doublereal *, doublereal *, doublereal *), 
	    dcoeft_(doublereal *, doublereal *, doublereal *, doublereal *, 
	    doublereal *, doublereal *, integer *, doublereal *, doublereal *,
	     logical *), tridia_(integer *, doublereal *, doublereal *, 
	    doublereal *, doublereal *), cowind_(doublereal *, doublereal *, 
	    doublereal *, integer *, doublereal *, doublereal *, doublereal *,
	     doublereal *, doublereal *, doublereal *), getqua_(doublereal *, 
    ...
    extern /* Subroutine */ int setupv_(doublereal *, doublereal *, 
	    doublereal *, doublereal *, doublereal *, doublereal *, 
	    doublereal *, integer *, integer *, logical *, integer *, 
	    doublereal *, doublereal *), ndifcom_(doublereal *, doublereal *, 
	    doublereal *, doublereal *, doublereal *, doublereal *, 
	    doublereal *, doublereal *, doublereal *, doublereal *, 
	    doublereal *, doublereal *, doublereal *, integer *, integer *, 
	    integer *, integer *, logical *, logical *, integer *, integer *, 
	    doublereal *, integer *, integer *), rotgrid_(doublereal *, 
	    doublereal *, doublereal *, doublereal *, doublereal *, 
	    doublereal *, doublereal *, doublereal *, doublereal *, 
	    doublereal *, doublereal *, doublereal *, doublereal *, integer *,
    ...
    static cilist io___56 = { 0, 0, 0, fmt_199, 0 };
...
/*  OUTPUT OF THE MODEL IS GENERATED. */
    ...
    /* Function Body */
    if (! rotprt_1.lprt0) {
	lprt = (*model + 1) % ccout1_1.nprt1 == 0 || (*model + 1) % 
		ccout1_1.nprt2 == 0;
    } else {
	lprt = TRUE_;
    }
    if (! ccout_1.lscrib || ! ccout_1.lprtin) {
	lprt = FALSE_;
    }
/* MHP 9/94 */
...
/* MHP 06/02 added timestep to call */
    vcirc_(&hjm[1], &hr[1], hru, imin, imax, &it, &lcz[1], m, &omega[1], &ldo,
...
/* MHP 06/02 added pressure and timestep to call */
	    vcirc_(&hjm[1], &hr[1], hru, imin, imax, &it, &lcz[1], m, &omega[
		    1], &ldo, qwlnr, qwrmax, &hv[1], &cltot, dt, &hp[1]);
	    if (varfc_1.lvfc) {
		getfc_(&hd[1], hru, &hv[1], imin, imax, &omega[1]);
	    }
...
/* MHP 10/02 UNUSED LFIRST REMOVED FROM CALL */
	    wind_(bl, dt, hsbot, hstop, imax, m, &ljdot, omegas, sjtot, smass,
		     teffl, hicz, &hjm[1]);
/*     *                SJTOT,SMASS,TEFFL,HICZ,HJM,LFIRST) */
/* REMOVE TORQUE FROM ENTIRE STAR */
	} else if (*lczsur && sbrot_1.lsolid) {
	    ljdot = cwind_1.ljdot0;
	    hsbotj = 0.;
	    hstopj = exp(const1_1.cln * *hstot);
	    imaxj = 1;
	    wind_(bl, dt, &hsbotj, &hstopj, &imaxj, m, &ljdot, omegas, sjtot, 
...
/*  FIND DIFFUSION COEFFICIENTS(COD) FOR ALL UNSTABLE REGIONS. */
	codiff_(delts, hru, &hv[1], imin, imax, &lc[1], &lcz[1], m, rmid, cod,
		 &cod2[1]);
...
/*  IF SO,CALCULATE TERMS FOR DIFFUSION. */
		...
		if (lock) {
			...
			*omegas = omega[*m];
			cowind_(bl, dt, hicz, &it, omegas, smass, teffl, &
				wbeg, &wind1, &wind2);
		    } else {
			wind1 = 0.;
			wind2 = 0.;
...
/*  EQUALLY SPACED GRID POINTS. */
		tridia_(&egrid_1.ntot, ei, ej, dj, &sumdj);
...
/* SOLVE FOR OMEGA AND ITS DERIVATIVES IN A BAND MATRIX */
		dadcoeft_(&dr, dt, ei, ej, ew, &egrid_1.ntot, &wind1, &wind2, 
			dj, ecod2, &sumdj, &lfix, &lokad);
	    }
...
/*         IF(M.GT.1)THEN */
	...
	ndifcom_(dt, &cod2[1], ecod2, em, &hd[1], &hl[1], &hp[1], &hr[1], &hs[
		1], &hs1[1], &hs2[1], hstot, &hv[1], &ibeg, &iend, imax, imin,
		 &lcz[1], lok, m, &egrid_1.ntot, &hcomp[16], &jbeg, &jend);
/*         ELSE */
...
/*         WRITE(*,*)LSOLID,OMEGA(1),OMEGA(M) */
	checkj_(&hd[1], hjmp, &hjmsav[1], &hr[1], &hs[1], &hs2[1], &hv[1], 
		imin, imax, &it, &lcz[1], &lprt, m, dt, &eta2[1], &hi[1], &
		hjm[1], iredo, lok, lredo, &omega[1], &qiw[1], &r0[1], wsav, 
		id, &idm, &lokad);
...
/*      IF(M.GT.1)THEN */
    ...
    ndifcom_(dt, &cod2[1], ecod2, em, &hd[1], &hl[1], &hp[1], &hr[1], &hs[1], 
	    ...
	    , lok, m, &egrid_1.ntot, &hcomp[16], &jbeg, &jend);
/*      ELSE */
/*      CALL DIFCOM(DR,DT,COD2,ECOD2,EM,ES1,HRU,HS,HS1,HS2,HV, */
/*     *            IBEG,IEND,IMIN,LCZ,LOK,M,NTOT,HCOMP) */
/*      ENDIF */
