/* mix.f -- translated by f2c (version 20100827).
...
/* Subroutine */ int mix_(doublereal *delts, doublereal *hcomp, doublereal *
	...
	integer *itlvl, logical *lc, integer *m, doublereal *qdt, doublereal *
	qdp, doublereal *ddage, integer *jcore, integer *jenv, integer *
	mxzone, integer *mxzon0)
{

    /* Format strings */
    static char fmt_110[] = "(\002 ZONES MIXED IN ORDER--\002,12(\002(\002,i"
	    "3,\002,\002,i3,\002) \002))";
    ...
    extern /* Subroutine */ int kemcom_(doublereal *, integer *, integer *, 
	    ...
	    doublereal *, doublereal *, doublereal *, doublereal *, 
	    doublereal *, doublereal *, doublereal *, doublereal *, 
	    doublereal *, integer *), convec_(doublereal *, doublereal *, 
    ...
    /* Fortran I/O blocks */
    static cilist io___58 = { 0, 0, 0, fmt_110, 0 };
    static cilist io___61 = { 0, 0, 0, fmt_911, 0 };
...
/* SETUP NUCLEAR ENERGY TERMS */
	rates_(&dl, &tl, &x, &y, &z__, &xhe3, &xc12, &xc13, &xn14, &xn15, &
		xo16, &xo17, &xo18, &xh2, &xli6, &xli7, &xbe9, &i__, hr1, hr2,
		 hr3, hr4, hr5, hr6, hr7, hr8, hr9, hr10, hr11, hr12, hr13, 
		hf1, hf2);
/* MHP 5/02 COMPUTE RATE FOR DEUTERIUM BURNING */
	if (xh2test > 1e-11) {
	    deutrate_(&dl, &tl, &x, &xh2, &i__, itlvl);
	} else {
...
/* EXIT LOOP ONCE T DROPS BELOW NUCLEAR REACTION T CUTOFF */
	    ...
	    kemcom_(&ht[1], &ibegin, &iend, hr1, hr2, hr3, hr4, hr5, hr6, hr7,
		     hr8, hr9, hr10, hr11, hr12, hr13, hf1, &hs2[1], &hcomp[
		    16], ddage, itlvl);
/* L30: */
	}
/* L40: */
...
/* NOTE KEMCOM ALSO AUTOMATICALLY HOMOGENIZE CONVECTION ZONES. */
	...
	iend = mxzone[k + 24];
	kemcom_(&ht[1], &ibegin, &iend, hr1, hr2, hr3, hr4, hr5, hr6, hr7, 
		hr8, hr9, hr10, hr11, hr12, hr13, hf1, &hs2[1], &hcomp[16], 
		ddage, itlvl);
/* L50: */
    }
/* L51: */
...
/* EXIT LOOP ONCE T DROPS BELOW NUCLEAR REACTION T CUTOFF */
		...
		ibegin = j;
		iend = j;
		eqburn_(hf1, hr1, hr2, hr3, hr4, hr5, hr6, hr7, hr8, hr9, 
			hr10, hr11, hr12, &hs2[1], &ht[1], &ibegin, &iend, &
			dcdt, &dodt, &dxdt, &dydt, &xc12, &xo16, &x, &z__);
...
/* NOTE KEMCOM ALSO AUTOMATICALLY HOMOGENIZE CONVECTION ZONES. */
	...
	for (k = 1; k <= i__1; ++k) {
	    ibegin = mxzone[k + 12];
	    iend = mxzone[k + 24];
	    eqburn_(hf1, hr1, hr2, hr3, hr4, hr5, hr6, hr7, hr8, hr9, hr10, 
		    hr11, hr12, &hs2[1], &ht[1], &ibegin, &iend, &dcdt, &dodt,
