/* getopac.f -- translated by f2c (version 20100827).
...
/*     HELIUM BURNING REGION (HB EVOLUTION) USE PURE Z TABLE */
    if (*z__ > .15 || (d__1 = *z__ - comp_1.zenv, abs(d__1)) > optab_1.optol 
	    && ! newopac_1.l2z) {
	if (! nwlaol_1.lpurez) {
	    io___11.ciunit = luout_1.ishort;
	    s_wsle(&io___11);
	    do_lio(&c__9, &c__1, " ERROR: Z.NE.ZENV. NEED PURE Z", (ftnlen)30)
		    ;
	    do_lio(&c__9, &c__1, " TABLE TO CONTINUE. Z,ZENV=", (ftnlen)27);
	    do_lio(&c__5, &c__1, (char *)&(*z__), (ftnlen)sizeof(doublereal));
	    do_lio(&c__5, &c__1, (char *)&comp_1.zenv, (ftnlen)sizeof(
		    doublereal));
	    e_wsle();
	    s_stop("", (ftnlen)0);
	}
