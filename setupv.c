/* setupv.f -- translated by f2c (version 20100827).
...
/* Subroutine */ int setupv_(doublereal *hd, doublereal *hg, doublereal *hl, 
	...
	integer *imin, integer *imax, logical *lcz, integer *m, doublereal *
	hru, doublereal *qwrmax)
{
    ...
    static doublereal f1, f2, f3, v0, a21, a31, a41, a42, a43, a32, cg, ff, 
	    em, fl, dr, cg2, xa1, xa2, xa3, xa4, fac, eta, cpm[5000], qdr, 
	    opm[5000], htm, htp, fnu, qc1r, htp2, ddel, dmin__, hpsc, htsc, 
...
/*  TREATMENT OF FIRST INTERFACE. */
	...
	intvar_1.qdtm[1] = mdphy_1.sqdt[0] * intfac_1.faci[4] + mdphy_1.sqdt[
		1] * intfac_1.faci[5] + mdphy_1.sqdt[2] * intfac_1.faci[6] + 
		mdphy_1.sqdt[3] * intfac_1.faci[7];
	intvar_1.hs3[1] = hs1[1] * intfac_1.faci[4] + hs1[2] * intfac_1.faci[
		5] + hs1[3] * intfac_1.faci[6] + hs1[4] * intfac_1.faci[7];
	intvar_1.epsilm[1] = mdphy_1.sesum[0] * intfac_1.faci[4] + 
		mdphy_1.sesum[1] * intfac_1.faci[5] + mdphy_1.sesum[2] * 
		intfac_1.faci[6] + mdphy_1.sesum[3] * intfac_1.faci[7];
...
/*  specific heat */
	cpm[1] = mdphy_1.cp[0] * intfac_1.faci[4] + mdphy_1.cp[1] * 
		intfac_1.faci[5] + mdphy_1.cp[2] * intfac_1.faci[6] + 
		mdphy_1.cp[3] * intfac_1.faci[7];
    } else {
	i0 = *imin;
    }
/*  TREATMENT OF LAST INTERFACE. */
	...
	intvar_1.qdtm[*m - 1] = mdphy_1.sqdt[*m - 4] * intfac_1.faci[(*m << 2)
		 - 4] + mdphy_1.sqdt[*m - 3] * intfac_1.faci[(*m << 2) - 3] + 
		mdphy_1.sqdt[*m - 2] * intfac_1.faci[(*m << 2) - 2] + 
		mdphy_1.sqdt[*m - 1] * intfac_1.faci[(*m << 2) - 1];
	intvar_1.hs3[*m - 1] = hs1[*m - 3] * intfac_1.faci[(*m << 2) - 4] + 
		hs1[*m - 2] * intfac_1.faci[(*m << 2) - 3] + hs1[*m - 1] * 
		intfac_1.faci[(*m << 2) - 2] + hs1[*m] * intfac_1.faci[(*m << 
		2) - 1];
	intvar_1.epsilm[*m - 1] = mdphy_1.sesum[*m - 4] * intfac_1.faci[(*m <<
		 2) - 4] + mdphy_1.sesum[*m - 3] * intfac_1.faci[(*m << 2) - 
		3] + mdphy_1.sesum[*m - 2] * intfac_1.faci[(*m << 2) - 2] + 
		mdphy_1.sesum[*m - 1] * intfac_1.faci[(*m << 2) - 1];
...
/*  specific heat */
	cpm[*m - 1] = mdphy_1.cp[*m - 4] * intfac_1.faci[(*m << 2) - 4] + 
		mdphy_1.cp[*m - 3] * intfac_1.faci[(*m << 2) - 3] + 
		mdphy_1.cp[*m - 2] * intfac_1.faci[(*m << 2) - 2] + 
		mdphy_1.cp[*m - 1] * intfac_1.faci[(*m << 2) - 1];
    } else {
	i1 = *imax;
    }
...
/*  D LN RHO/D LN T. */
	intvar_1.qdtm[i__ - 1] = mdphy_1.sqdt[i__ - 3] * intfac_1.faci[(i__ <<
		 2) - 4] + mdphy_1.sqdt[i__ - 2] * intfac_1.faci[(i__ << 2) - 
		3] + mdphy_1.sqdt[i__ - 1] * intfac_1.faci[(i__ << 2) - 2] + 
		mdphy_1.sqdt[i__] * intfac_1.faci[(i__ << 2) - 1];
...
/*  SPECIFIC ENERGY GENERATION RATE. */
	intvar_1.epsilm[i__ - 1] = mdphy_1.sesum[i__ - 3] * intfac_1.faci[(
		i__ << 2) - 4] + mdphy_1.sesum[i__ - 2] * intfac_1.faci[(i__ 
		<< 2) - 3] + mdphy_1.sesum[i__ - 1] * intfac_1.faci[(i__ << 2)
		 - 2] + mdphy_1.sesum[i__] * intfac_1.faci[(i__ << 2) - 1];
/*  LUMINOSITY. */
...
/*  specific heat */
	cpm[i__ - 1] = mdphy_1.cp[i__ - 3] * intfac_1.faci[(i__ << 2) - 4] + 
		mdphy_1.cp[i__ - 2] * intfac_1.faci[(i__ << 2) - 3] + 
		mdphy_1.cp[i__ - 1] * intfac_1.faci[(i__ << 2) - 2] + 
		mdphy_1.cp[i__] * intfac_1.faci[(i__ << 2) - 1];
/* L30: */
    }
    i__1 = *imax;
...
/*  ALL INTERFACES. */
    ...
    for (i__ = *imin; i__ <= i__1; ++i__) {
	if (! varfc_1.ldifad) {
	    delmu = (d_lg10(&mdphy_1.hamu[i__ - 1]) - d_lg10(&mdphy_1.hamu[
...
/*        TTHERM = 8.0d0*CC23*CSIG*TM(I)**3/OPM(I)/DM(I)**2/CPM(I) */
	...
	ff = intvar_1.pm[i__ - 1] / (intvar_1.hgm[i__ - 1] * ddel * cpm[i__ - 
...
/* Computing MAX */
	d__1 = intvar_1.delami[i__ - 1] - intvar_1.delmi[i__ - 1];
	vfact_1.fact1[i__ - 1] = 1. / intvar_1.qdtm[i__ - 1] / max(d__1,.001);
...
/* Computing 2nd power */
	    ...
	    f1 = intvar_1.pm[i__ - 1] / (intvar_1.hgm[i__ - 1] * 
		    oldrot2_1.ddelm[i__ - 1] * cpm[i__ - 1] * intvar_1.dm[i__ 
		    - 1] * intvar_1.tm[i__ - 1]);
