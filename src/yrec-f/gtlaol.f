...
C     RTL2 = TL-FACTOR
      RDL = DL
      CALL LOCATE(OT,NUMT,RTL,JT)
      CALL LOCATE(OXA,NUMOFXYZ,RX,JX)
      IF (JX .EQ. NUMOFXYZ) THEN
          JX = NUMOFXYZ-1
      END IF
      IF (JX .EQ. 0) THEN
          JX = 1
...
C        GET RANGE OF FOUR TT SURROUNDING T
         CALL XRNG4(JT,NUMT,JT1,JT2)
         DO IT=JT1, JT2
            NUMS = NUMRS(IX,IT)
            IF(NUMS .GE. 4) THEN
               DO IR=1, NUMS
                  TOP(IR) = SLAOLL(IX,IR,IT)
                  TR(IR) = SRHOL(IX,IR,IT)
                  TDOR(IR) = SDORL(IX,IR,IT)
               END DO
