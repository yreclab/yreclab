/* setup_grsett.f -- translated by f2c (version 20061008).
	...
	doublereal *qcod2x)
{
    ...
    static integer i__;
    static doublereal t, ln_lambda__, x, z__, crsun_bah__, x2, x3, ac, ah, ao,
	     ap;
    static integer ii;
    static doublereal ne;
    static integer jj;
    static doublereal at, ni, cz, csecyr_bah__, xz, zz, fac, tac[16]	/* 
	    was [4][4] */, tcl[16]	/* was [4][4] */, tap[4], tat[4], rho,
	     xij, zxa, fac2, qacx, qapx, qatx, xfrac[4], lambda;
    ...
    static cilist io___8 = { 0, 0, 0, fmt_16, 0 };
    static cilist io___9 = { 0, 0, 0, fmt_17, 0 };
...
/*                 calculate Coulomb logarithms: */
	    ...
	    qacx = x * 1.294 - 1.42;
/* FD 10/09 Mimic Mixing to reduce settling. */
/*            COD1(I) = FAC*HQPR(I)*X*(AP+AT) */
	    cod1[i__] = cmixing_1.cstmixing * fac * hqpr[i__] * x * (ap + at);
	    cod2[i__] = cmixing_1.cstmixing * fac * ac;
...
/* FD 10/09 Mimic Mixing to reduce settling (cstmixing) */
/*         and add the uncertainties of differential mixing (cstdiffmix). */

/* old ver      COD1Z(I) = FAC*HQPR(I)*ZZ*(AP+AT) */
		gravsz_1.cod1z[i__ - 1] = cmixing_1.cstdiffmix * 
			cmixing_1.cstmixing * fac * hqpr[i__] * zz * (ap + at)
			;
/*              POSITIVE DIFFUSION COEFFICIENTS NEEDED! */
/* old ver.     COD2Z(I) = ABS(FAC*AH) */
/* old ver.     QCOD1Z(I) = FAC*HQPR(I)*(AP+AT) */
		gravsz_1.cod2z[i__ - 1] = cmixing_1.cstmixing * (d__1 = fac * 
			ah, abs(d__1));
		gravsz_1.qcod1z[i__ - 1] = cmixing_1.cstmixing * fac * hqpr[
			i__] * (ap + at);
