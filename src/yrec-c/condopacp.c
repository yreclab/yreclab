/* condopacp.f -- translated by f2c (version 20061008).
...
/*      if (IT.eq.0.or.IT.eq.MAXT) stop'CONINTER: T out of range' */
    if (it == 0 || it == 19) {
	s_wsle(&io___15);
	do_lio(&c__5, &c__15, (char *)&az[0], (ftnlen)sizeof(doublereal));
	e_wsle();
	s_wsle(&io___16);
	do_lio(&c__3, &c__1, (char *)&c__15, (ftnlen)sizeof(integer));
	do_lio(&c__5, &c__1, (char *)&(*tlg), (ftnlen)sizeof(doublereal));
	do_lio(&c__3, &c__1, (char *)&iz, (ftnlen)sizeof(integer));
	e_wsle();
