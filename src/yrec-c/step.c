/* step.f -- translated by f2c (version 20200916).
...
struct state_1_ {
    logical linit, lpunch;
    integer modeln;
    logical lkindinit;
};

#define state_1 (*(struct state_1_ *) &state_)
...
/* $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ */
/* STEP: Execute a single YREC timestep. */
/* LC 11/25 */
/* $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ */

/* Note: LCALST, LCALS, LMONTE are disabled. */
/* These modify the standard run timings, or represent obsolete */
/* functionality with a driver function. */
...
/* Subroutine */ int step_(void)
{
    ...
    extern /* Subroutine */ int massloss_(doublereal *, doublereal *, 
	    ...
	    integer *, doublereal *, doublereal *, doublereal *, doublereal *,
	     logical *), putstore_(doublereal *, doublereal *, doublereal *, 
    ...
    static doublereal hg[5000], hi[5000], bl, dl, hl[5000], fp[5000], hp[5000]
	    , hr[5000], hs[5000], ht[5000], ft[5000], ps[3], rs[3], ts[3], tl;
    static integer iu;
    ...
    static doublereal qdp, qdt, be7m[5000], eta2[5000], vel, pdt, pdp, xc12, 
	    xc13, xn14, xn15, xhe3, xo16, xo17, xo18, epp1, epp2, epp3, e3al, 
    ...
    static logical lage, lsbc;
    static doublereal hmax[4], hsrf[6], tril[3], trit[3], ftri;
    ...
    static doublereal facage;
    static logical lsavpu, lreset;
    ...
    /* Fortran I/O blocks */
    static cilist io___56 = { 0, 6, 0, fmt_101, 0 };
    static cilist io___57 = { 0, 0, 0, fmt_101, 0 };
    static cilist io___58 = { 0, 6, 0, fmt_102, 0 };
    static cilist io___59 = { 0, 0, 0, fmt_102, 0 };
    static cilist io___60 = { 0, 6, 0, fmt_103, 0 };
    static cilist io___61 = { 0, 0, 0, fmt_103, 0 };
    static cilist io___111 = { 0, 6, 0, fmt_911, 0 };
    static cilist io___112 = { 0, 76, 0, fmt_222, 0 };
    static cilist io___117 = { 0, 76, 0, fmt_145, 0 };
    static cilist io___130 = { 0, 6, 0, 0, 0 };
    static cilist io___131 = { 0, 6, 0, 0, 0 };
    static cilist io___173 = { 0, 6, 0, fmt_104, 0 };
    static cilist io___174 = { 0, 6, 0, fmt_105, 0 };
    static cilist io___175 = { 0, 6, 0, fmt_106, 0 };
...
/* LC 11/25 Execution state for YREC: */
...
/* MHP 8/25 directly pass file names instead of using common blocks */
	setups_(v, nmlctrl_1.falex06, nmlctrl_1.fallard, nmlctrl_1.fatm, 
		...
		nmlctrl_1.fscvhe, nmlctrl_1.fscvz, nmlctrl_1.opecalex, (
		ftnlen)256, (ftnlen)256, (ftnlen)256, (ftnlen)256, (ftnlen)
		256, (ftnlen)256, (ftnlen)256, (ftnlen)256, (ftnlen)256, (
		ftnlen)256, (ftnlen)256, (ftnlen)256, (ftnlen)256, (ftnlen)
		256, (ftnlen)256, (ftnlen)256, (ftnlen)256, (ftnlen)256, (
		ftnlen)256, (ftnlen)256, (ftnlen)256, (ftnlen)256, (ftnlen)
		256, (ftnlen)256);
...
/* ********** */
    if (zramp_1.nk <= ckind_1.numrun && ! state_1.lkindinit) {
...
/*        CALL STARIN(BL,CFENV,DAGE,DDAGE,DELTS,DELTSH,DELTS0,ETA2,  ! KC 2025-05-31 */
	starin_(&bl, cfenv, &theage_1.dage, &ddage, &delts, &deltsh, eta2, fp,
		 ft, &ftri, hcomp, hd, hi, hjm, hkerot, hl, hp, hr, hs, hs1, 
		hs2, &hstot, ht, &ikut, &istore, &jenv, &large, lc, &lnew, &m,
		 &model, &zramp_1.nk, omega, ps, &qdp, &qdt, qiw, r0, rs, &
		sjtot, &skerot, &smass, &teffl, tlumx, tril, trit, ts, &vel, 
		hg, v);
...
*/
	if (sett_1.lendag[zramp_1.nk - 1]) {
		...
		sett_1.end_dcen__[zramp_1.nk - 1] = -sett_1.end_dcen__[
			zramp_1.nk - 1];
		s_wsfe(&io___56);
		do_fio(&c__1, (char *)&hcomp[11], (ftnlen)sizeof(doublereal));
...
/* SKIP CALCULATIONS FOR LOW TEMPERATURES. */
		...
		xo17 = hcomp[i__ * 15 - 6];
		xo18 = hcomp[i__ * 15 - 5];
		engeb_(&epp1, &epp2, &epp3, &ecn, &e3al, &pep, &pet, &sum1, &
...
/* BY THE MASS OF EACH SHELL IN GM (HS2). */
		for (j = 1; j <= 10; ++j) {
		    fluxi0[j + i__ * 10 - 11] = fluxes_1.flux[j - 1] * hs2[
			    i__ - 1];
		...
		do_fio(&c__1, (char *)&hs2[i__ - 1], (ftnlen)sizeof(
			doublereal));
		for (j = 1; j <= 10; ++j) {
		    do_fio(&c__1, (char *)&fluxi0[j + i__ * 10 - 11], (ftnlen)
			    sizeof(doublereal));
		}
		e_wsfe();
	    }
...
/* FLUXES ARE PRINTED IN THE SAME ORDER AS BAHCALL AND PINSONNEAULT. */
		...
		do_fio(&c__1, (char *)&be7m[i__ - 1], (ftnlen)sizeof(
			doublereal));
		do_fio(&c__1, (char *)&fluxi0[i__ * 10 - 10], (ftnlen)sizeof(
			doublereal));
		do_fio(&c__1, (char *)&fluxi0[i__ * 10 - 6], (ftnlen)sizeof(
			doublereal));
		do_fio(&c__1, (char *)&fluxi0[i__ * 10 - 5], (ftnlen)sizeof(
			doublereal));
		do_fio(&c__1, (char *)&fluxi0[i__ * 10 - 4], (ftnlen)sizeof(
			doublereal));
		do_fio(&c__1, (char *)&fluxi0[i__ * 10 - 3], (ftnlen)sizeof(
			doublereal));
		do_fio(&c__1, (char *)&fluxi0[i__ * 10 - 7], (ftnlen)sizeof(
			doublereal));
		do_fio(&c__1, (char *)&fluxi0[i__ * 10 - 9], (ftnlen)sizeof(
			doublereal));
		do_fio(&c__1, (char *)&fluxi0[i__ * 10 - 8], (ftnlen)sizeof(
			doublereal));
		e_wsfe();
	    }
L141:
...
/* or it will crash - so copy initial model to unit ILAST */
		...
		wrtlst_(&iwrite, hcomp, hd, hl, hp, hr, hs, ht, lc, trit, 
			tril, ps, ts, rs, cfenv, &ftri, tlumx, &jcore, &jenv, 
			&model, &m, &smass, &teffl, &bl, &hstot, &
...
/*      *               JXMID,TLUMX,DAGE,DDAGE,QDT,QDP,NK,HP,HR,OMEGA,  ! KC 2025-05-31 */
	htimer_(&delts, &deltsh, &m, hd, hl, hs1, hs2, ht, hcomp, &jcore, &
		jxmid, tlumx, &theage_1.dage, &ddage, &zramp_1.nk, hp, hr, 
		omega, &dwmax, &jxbeg, &teffl);
	delts0 = delts;
...
/* zero out light element burning rates in the surface CZ. */
	...
	state_1.lkindinit = TRUE_;
    }
/* step a single time. LC 11/25 */
    if (state_1.modeln <= ckind_1.nmodls[zramp_1.nk - 1] && state_1.lkindinit)
	     {
...
/* DBG PULSE:  if last model of last run then set LPULSE to LSAVPU */
	if (state_1.modeln == ckind_1.nmodls[zramp_1.nk - 1] && zramp_1.nk == 
		ckind_1.numrun) {
...
/* Also, save model structure. */
	    if (nao < 6) {
		if (theage_1.dage + ddage / 1e9 - acdpth_1.ageout[nao - 1] <= 
			0. && theage_1.dage + ddage * 2. / 1e9 - 
			acdpth_1.ageout[nao - 1] >= 0. && ! acdpth_1.ljwrt) {
		    s_wsle(&io___131);
...
/* MHP 10/24 GENERALIZE CHECK */
	if (sett_1.lendag[zramp_1.nk - 1] && sett_1.endage[zramp_1.nk - 1] > 
		0. && (d__1 = sett_1.endage[zramp_1.nk - 1] - theage_1.dage * 
		1e9 - ddage, abs(d__1)) <= 1.) {
	    pulse_1.lpulse = lsavpu;
...
/* MHP 8/25 changed to add file names as declared variables */
	    pdist_(&pol1, &pot1, &poa1, &polen, &bl, &teffl, &state_1.modeln, 
...
/* ADD MASS LOSS CALCULATION */
	    massloss_(&bl, &theage_1.dage, &delts, hcomp, hd, hjm, hp, hr, hs,
...
/* L33: */
	    ...
	    itlvl = 1;
	    mix_(&delts, hcomp, hd, hl, hp, hr, hs, hs1, hs2, &hstot, ht, &
		    itlvl, lc, &m, &ddage, &jcore, &jenv, mxzone, mxzon0, &
		    teffl);
...
/* rezone new model, except rezoning not performed for He flash calculations */
	if (! heflsh_1.lkuthe) {
	    hpoint_(&m, &hstot, hs, hs1, hs2, ht, hp, hr, lc, hl, hd, hcomp, 
		    ps, rs, estore, &istore, &lreset, &jxbeg, &model, &lshell,
		     &jcore, &jenv, omega, eta2, r0, hi, hjm, hkerot, &sjtot, 
		    &skerot, fp, ft, hg, qiw, &teffl);
...
/* store starting depth of C.Z. for light element burning. */
	if (flag_1.lexcom) {
	    ...
	    jenv0 = jenv;
	    if (dpmix_1.lovste) {
		liov_1.pscal0 = dpmix_1.alphae * exp(const3_1.clndp * (hp[
			jenv - 1] + hr[jenv - 1] * 2. - hd[jenv - 1] - 
			const2_1.cgl - hs[jenv - 1]));
	    } else {
		liov_1.pscal0 = 0.;
	    }
...
/* CALL TO CRRECT - ADDED ITERATION LEVEL */
	itlvl = 1;
	crrect_(&delts, &m, &niter, &lfini, &large, &lnew, &lreset, &lsbc, 
		trit, tril, cfenv, ps, ts, rs, &ftri, &istore, estore, &hstot,
		 &bl, &teffl, hd, hha, hhb, hhc, hl, hmax, hp, hpp, hr, hs, 
		hs1, hs2, hsrf, ht, htt, hcomp, lc, nhmax, tlumx, &latmo, &
		lderiv, &lmix, &locond, &qdt, &qdp, fp, ft, eta2, omega, r0, &
		itdone, hg, hi, hjm, &itlvl, lcz, mxzone, qiw, hkerot, 
		hkeroto);
...
/* IMPROVED FIRST GUESS AT THE STRUCTURE FOR THE NEXT MODEL IF LNEWS=T. */
	if (delts > 0.) {
		...
		entrop_1.hll0[ii - 1] = (hl[ii - 1] - oldmod_1.hlo[ii - 1]) * 
			2. / (hl[ii - 1] + oldmod_1.hlo[ii - 1]) / delts;
		entrop_1.hrr0[ii - 1] = (hr[ii - 1] - oldmod_1.hro[ii - 1]) / 
			delts;
/* L27: */
	    }
	}
...
/* OVERWRITTEN ON THE LAST RUN THROUGH. */
	if (difus_1.itdif1 > 1) {
		...
		for (j = 1; j <= 15; ++j) {
		    origstart_1.hcompp0[j + i__ * 15 - 16] = oldmod_1.hcompp[
			    j + i__ * 15 - 16];
		}
	    }
	}
	i__1 = difus_1.itdif1;
	for (itrot = 1; itrot <= i__1; ++itrot) {
...
/* (to be implemented when I know the rest of it works!) */
	    ...
	    itlvl = 4;
	    crrect_(&delts, &m, &niter, &lfini, &large, &lnew, &lreset, &lsbc,
		     trit, tril, cfenv, ps, ts, rs, &ftri, &istore, estore, &
		    hstot, &bl, &teffl, hd, hha, hhb, hhc, hl, hmax, hp, hpp, 
		    hr, hs, hs1, hs2, hsrf, ht, htt, hcomp, lc, nhmax, tlumx, 
		    &latmo, &lderiv, &lmix, &locond, &qdt, &qdp, fp, ft, eta2,
		     omega, r0, &itdone, hg, hi, hjm, &itlvl, lcz, mxzone, 
		    qiw, hkerot, hkeroto);
/*      *           LMIX,LOCOND,QDT,QDP,MODEL,FP,FT,ETA2,OMEGA,R0,ITDONE, */
/*      *           HG,HI,HJM,ITLVL,LCZ,MRZONE,MXZONE,NRZONE,NZONE,  ! KC 2025-05-31 */
/*  25         CONTINUE */
...
/* JENV0 ADDED TO SR CALL. */
		...
		getw_(&bl, &delts, &dwmax, fp, ft, hcomp, hd, hjm, hl, hp, hr,
...
/* FIND CONVECTION ZONE DEPTH AT THE END OF THE TIME STEP. */
		convec_(hcomp, hd, hp, hr, hs, ht, lc, &m, mrzone, mxzone, 
			mxzon0, &jcore, &jenv, &nrzone, &nzone, &nzone0);
/* CHANGED FOR LITHIUM BURNING WITH OVERSHOOT. */
		jenv1 = jenv;
		if (dpmix_1.lovste) {
		    liov_1.pscal1 = dpmix_1.alphae * exp(const3_1.clndp * (hp[
			    jenv - 1] + hr[jenv - 1] * 2. - hd[jenv - 1] - 
			    const2_1.cgl - hs[jenv - 1]));
		} else {
		    liov_1.pscal1 = 0.;
...
/*                CALL LIBURN(DELTS,HCOMP,HD,HR,HS1,HS2,HT,JENV1,JENV0,M)  ! KC 2025-05-31 */
		liburn_(&delts, hcomp, hr, hs1, hs2, ht, &jenv1, &jenv0, &m);
	    }
	}
/* MHP 07/02 RESTORE PRIOR FITTING POINT IF MASS ACCRETION IS BEING */
/* INCLUDED */
	if (lnewfit) {
	    getnewenv_(&senvold, hcomp, hd, hl, hp, hr, hs, hs1, hs2, &hstot, 
		    ht, lc, eta2, hi, hjm, qiw, r0, hkerot, &bl, &sjtot, &
		    skerot, &teffl, &m, &lnew);
...
/* ONLY IF A FIXED END AGE IS USED, NOT FOR OTHER STOPS */
	if (sett_1.lendag[zramp_1.nk - 1] && sett_1.endage[zramp_1.nk - 1] > 
		0.) {
	    if (sett_1.endage[zramp_1.nk - 1] - theage_1.dage * 1e9 <= 1.) {
/* Computing MAX */
		d__1 = delts0, d__2 = theage_1.dage * .001 * const3_1.csecyr;
		delts = max(d__1,d__2);
		ddage = delts / const3_1.csecyr;
	    } else {
		delts0 = delts;
...
/* WRTOUT IS THE OUTPUT DRIVER ROUTINE */
	wrtout_(hcomp, hd, hl, hp, hr, hs, hs1, ht, lc, &m, &model, &
		...
		jxbeg, &jxmid, &jxend, &jcore, &jenv, tlumx, trit, tril, ps, 
		ts, rs, cfenv, &ftri, &hstot, omega, &state_1.lpunch, fp, ft, 
		eta2, r0, hjm, hi, &sjtot, &skerot, hs2);
...
/* MHP 10/24 CHECK ALL STOP CONDITIONS, EXIT IF ANY SATISFIED */
	...
	if (sett_1.lendag[zramp_1.nk - 1] && sett_1.end_dcen__[zramp_1.nk - 1]
	    ...
	    s_wsfe(&io___173);
	    do_fio(&c__1, (char *)&hcomp[11], (ftnlen)sizeof(doublereal));
	    do_fio(&c__1, (char *)&sett_1.end_dcen__[zramp_1.nk - 1], (ftnlen)
		    sizeof(doublereal));
	    e_wsfe();
	    lend_kind__ = TRUE_;
	}
	if (sett_1.lendag[zramp_1.nk - 1] && sett_1.end_xcen__[zramp_1.nk - 1]
		 > 0. && hcomp[0] < sett_1.end_xcen__[zramp_1.nk - 1]) {
	    s_wsfe(&io___174);
	    do_fio(&c__1, (char *)&hcomp[0], (ftnlen)sizeof(doublereal));
	    do_fio(&c__1, (char *)&sett_1.end_xcen__[zramp_1.nk - 1], (ftnlen)
		    sizeof(doublereal));
	    e_wsfe();
	    lend_kind__ = TRUE_;
	}
	if (sett_1.lendag[zramp_1.nk - 1] && sett_1.end_ycen__[zramp_1.nk - 1]
		 > 0. && hcomp[1] < sett_1.end_ycen__[zramp_1.nk - 1]) {
	    s_wsfe(&io___175);
	    do_fio(&c__1, (char *)&hcomp[1], (ftnlen)sizeof(doublereal));
	    do_fio(&c__1, (char *)&sett_1.end_ycen__[zramp_1.nk - 1], (ftnlen)
		    sizeof(doublereal));
	    e_wsfe();
	    lend_kind__ = TRUE_;
	}
/* IF EXITING, SET I/O FLAGS PROPERLY AND EXIT LOOP */
	if (lend_kind__) {
	    ...
	    sound_1.lsound = TRUE_;
	    rotprt_1.lprt0 = TRUE_;
/* End execution states LC 11/25 */
	    state_1.modeln = ckind_1.nmodls[zramp_1.nk - 1];
	}
/* END OF RUN */
...
L110:
	if (ccout_1.lstore && ccout2_1.lstpch && state_1.lpunch) {
	    putstore_(hcomp, hd, hl, hp, hr, hs, ht, lc, trit, tril, ps, ts, 
		    ...
		    smass, &teffl, &bl, &hstot, &theage_1.dage, &ddage, omega,
		     hs1, eta2, r0, fp, ft, hjm, hi);
	    state_1.lpunch = FALSE_;
	}
/* 110  CONTINUE */
/* G Somers END */
