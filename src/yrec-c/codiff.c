/* codiff.f -- translated by f2c (version 20061008).
...
/* $$$$$$ */
/*       SUBROUTINE CODIFF(DELTS,HRU,HV,IMIN,IMAX,LC,LCZ,M,RMID,COD,COD2)  ! KC 2025-05-31 */
/* Subroutine */ int codiff_(doublereal *hru, integer *m, doublereal *rmid, 
	doublereal *cod, doublereal *cod2)
{
...
/* MHP 9/93 */
/*       DIMENSION HRU(JSON),HV(JSON),COD(JSON),LC(JSON),LCZ(JSON),  ! KC 2025-05-31 */
...
/* MHP 9/14 ADDED LOOP TO ALLOW A CONSTANT BACKGROUND DIFFUSION COEFFICIENT */
	...
	if (! varfc_1.lvfc) {
	    ...
	    i__1 = *m;
	    for (i__ = 2; i__ <= i__1; ++i__) {
		cod[i__] = con1 * (temp2_1.ves[i__ - 1] + temp2_1.vgsf[i__ - 
			1] + temp2_1.vss[i__ - 1]) * rmid[i__];
		cod2[i__] = con2 * (vmult2_1.fesc * temp2_1.ves[i__ - 1] + 
			vmult2_1.fgsfc * temp2_1.vgsf[i__ - 1] + 
			vmult2_1.fssc * temp2_1.vss[i__ - 1]) * rmid[i__];
/*               HLE(I)=RMID(I) */
	    }
	} else {
	    i__1 = *m;
...
/*               HLE(I) = RMID(I) */
	...
	if (! varfc_1.lvfc) {
	    ...
	    i__1 = *m;
	    for (i__ = 2; i__ <= i__1; ++i__) {
		cod[i__] = con1 * (mag_1.codm + (temp2_1.ves[i__ - 1] + 
			temp2_1.vgsf[i__ - 1] + temp2_1.vss[i__ - 1]) * rmid[
			i__]);
		cod2[i__] = con2 * (vmult2_1.fesc * temp2_1.ves[i__ - 1] + 
			vmult2_1.fgsfc * temp2_1.vgsf[i__ - 1] + 
			vmult2_1.fssc * temp2_1.vss[i__ - 1]) * rmid[i__];
