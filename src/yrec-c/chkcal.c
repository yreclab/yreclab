/* chkcal.f -- translated by f2c (version 20061008).
...
/* ADD CHECK FOR Z/X */
	if (cals2_1.lcalsolzx && cals2_1.calsolzx > 0.) {
	    zl = d_lg10(solzx) - d_lg10(&cals2_1.calsolzx);
	    if (abs(zl) < cals2_1.tolz) {
		calsun_1.lsol = TRUE_;
		goto L9999;
	    }
	} else {
...
/*     GET IMPROVED GUESSES FOR ALPHA AND X. */
    ...
    newxym_1.cmixla[*n + 1] = newxym_1.cmixla[*n];
    newxym_1.cmixla[*n + 2] = newxym_1.cmixla[*n];
    if (cals2_1.lcalsolzx) {
	ckind_1.rescal[(*n + 1 << 2) - 2] = ckind_1.rescal[(*n - 2 << 2) - 2] 
		* cals2_1.calsolzx / *solzx;
	dz = ckind_1.rescal[(*n + 1 << 2) - 2] - ckind_1.rescal[(*n - 2 << 2) 
		- 2];
	newxym_1.zenv0a[*n] = ckind_1.rescal[(*n + 1 << 2) - 2];
	newxym_1.zenv0a[*n + 1] = ckind_1.rescal[(*n + 1 << 2) - 2];
	newxym_1.zenv0a[*n + 2] = ckind_1.rescal[(*n + 1 << 2) - 2];
    }
	...
	s_wsle(&io___10);
	do_lio(&c__9, &c__1, "New Z, Old Z, DZ: ", (ftnlen)18);
	do_lio(&c__5, &c__1, (char *)&ckind_1.rescal[(*n + 1 << 2) - 2], (
		ftnlen)sizeof(doublereal));
	do_lio(&c__5, &c__1, (char *)&ckind_1.rescal[(*n - 2 << 2) - 2], (
		ftnlen)sizeof(doublereal));
