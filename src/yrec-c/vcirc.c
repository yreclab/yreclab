/* vcirc.f -- translated by f2c (version 20061008).
...
/* Subroutine */ int vcirc_(doublereal *hr, doublereal *hru, integer *imin, 
	integer *imax, integer *it, logical *lcz, integer *m, doublereal *
	omega, logical *ldo, doublereal *qwlnr, doublereal *qwrmax, 
	doublereal *hv, doublereal *cltot, doublereal *dt, doublereal *hp)
{
    /* Format strings */
    ...
    static doublereal q0, q1, q2, v2, dr, qp, qq, fx, qw, fx2, qmu, qqq, fxx, 
	    qwr, phi2, fxx2, ddel, gmid, rmid, wmid, wmin, dell, phis2, qwrmx,
	     qwrmx2, ddtest, phistd, dlnwdr, ddtest2, dlnwdr0, dlnjmdr, 
	    dlnjmdr0;

    /* Fortran I/O blocks */
    static cilist io___10 = { 0, 6, 0, fmt_303, 0 };
    static cilist io___35 = { 0, 6, 0, fmt_9911, 0 };
...
/* TERMS */
/*       DIMENSION HJM(JSON),HR(JSON),HRU(JSON),LCZ(JSON),OMEGA(JSON),  ! KC 2025-05-31 */
/* SPEED OF LIGHT. */
...
/*  TREATMENT OF FIRST INTERFACE. */
    if (*imin < 3) {
	intvr2_1.viscmi[1] = exp(log(mdphy_1.viscm[0]) * intfac_1.faci[4] + 
		log(mdphy_1.viscm[1]) * intfac_1.faci[5] + log(mdphy_1.viscm[
		2]) * intfac_1.faci[6] + log(mdphy_1.viscm[3]) * 
		intfac_1.faci[7]);
	intvr2_1.thdifmi[1] = exp(log(mdphy_1.thdifm[0]) * intfac_1.faci[4] + 
		log(mdphy_1.thdifm[1]) * intfac_1.faci[5] + log(
		mdphy_1.thdifm[2]) * intfac_1.faci[6] + log(mdphy_1.thdifm[3])
		 * intfac_1.faci[7]);
	i0 = 3;
    } else {
	i0 = *imin;
...
/*  TREATMENT OF LAST INTERFACE. */
    if (*imax == *m) {
	intvr2_1.viscmi[*m - 1] = exp(log(mdphy_1.viscm[*m - 4]) * 
		intfac_1.faci[(*m << 2) - 4] + log(mdphy_1.viscm[*m - 3]) * 
		intfac_1.faci[(*m << 2) - 3] + log(mdphy_1.viscm[*m - 2]) * 
		intfac_1.faci[(*m << 2) - 2] + log(mdphy_1.viscm[*m - 1]) * 
		intfac_1.faci[(*m << 2) - 1]);
	intvr2_1.thdifmi[*m - 1] = exp(log(mdphy_1.thdifm[*m - 4]) * 
		intfac_1.faci[(*m << 2) - 4] + log(mdphy_1.thdifm[*m - 3]) * 
		intfac_1.faci[(*m << 2) - 3] + log(mdphy_1.thdifm[*m - 2]) * 
		intfac_1.faci[(*m << 2) - 2] + log(mdphy_1.thdifm[*m - 1]) * 
		intfac_1.faci[(*m << 2) - 1]);
	i1 = *m - 1;
    } else {
	i1 = *imax;
...
/*  GENERAL CASE. */
	...
	intvr2_1.viscmi[i__ - 1] = exp(log(mdphy_1.viscm[i__ - 3]) * 
		intfac_1.faci[(i__ << 2) - 4] + log(mdphy_1.viscm[i__ - 2]) * 
		intfac_1.faci[(i__ << 2) - 3] + log(mdphy_1.viscm[i__ - 1]) * 
		intfac_1.faci[(i__ << 2) - 2] + log(mdphy_1.viscm[i__]) * 
		intfac_1.faci[(i__ << 2) - 1]);
	intvr2_1.thdifmi[i__ - 1] = exp(log(mdphy_1.thdifm[i__ - 3]) * 
		intfac_1.faci[(i__ << 2) - 4] + log(mdphy_1.thdifm[i__ - 2]) *
		 intfac_1.faci[(i__ << 2) - 3] + log(mdphy_1.thdifm[i__ - 1]) 
		* intfac_1.faci[(i__ << 2) - 2] + log(mdphy_1.thdifm[i__]) * 
		intfac_1.faci[(i__ << 2) - 1]);
/* L20: */
    }
/* USE LINEAR INTERPOLATION FOR OMEGA AND MU. */
	...
	intvr2_1.amumi[i__ - 1] = (mdphy_1.amum[i__ - 1] + mdphy_1.amum[i__ - 
		2]) * .5;
...
/* MHP 06/02 REPLACE WITH THE ZAHN&MAEDER 1998 PRESCRIPTION */
    i__1 = *imax;
    for (i__ = *imin; i__ <= i__1; ++i__) {
	qmu = log(mdphy_1.amum[i__ - 1]) - log(mdphy_1.amum[i__ - 2]);
...
/* MU GRADIENTS. */
	if (vmult2_1.igsf == 2 || vmult2_1.igsf == 0) {
	    qwrmx = sqrt(intvr2_1.viscmi[i__ - 1] / intvr2_1.thdifmi[i__ - 1])
		     * 2. * qwrmax[i__];
	...
	dr = hru[i__] - hru[i__ - 1];
	if (vmult2_1.igsf == 0) {
	    qwrmx = sqrt(intvar_1.hgm[i__ - 1] * (d__1 = mdphy_1.amum[i__ - 1]
		     - mdphy_1.amum[i__ - 2], abs(d__1)) / dr / 
		    intvr2_1.amumi[i__ - 1]) * 2.;
	    if ((d__1 = qwlnr[i__], abs(d__1)) < qwrmx) {
		temp2_1.vgsf[i__ - 1] = 0.;
		goto L40;
	    } else {
...
/* Computing 2nd power */
	    ...
	    temp2_1.vgsf[i__ - 1] = taukh_1.fgsfk[i__ - 1] * fx * 
		    intvr2_1.thdifmi[i__ - 1] * (d__1 * d__1) / rmid;
	} else {
/* Computing 2nd power */
	    d__1 = intvr2_1.wm[i__ - 1];
	    temp2_1.vgsf[i__ - 1] = taukh_1.fgsfk[i__ - 1] * .25 * 
		    intvr2_1.thdifmi[i__ - 1] * dlnwdr * (d__1 * d__1) / rmid;
	}
...
/* Computing 2nd power */
	    d__1 = qwr / qwrmax[i__];
	    temp2_1.vss[i__ - 1] = intvr2_1.thdifmi[i__ - 1] * 
...
/*  UNSTABLE; CHECK FOR MU GRADIENTS. */
	    if ((d__1 = (mdphy_1.amum[i__ - 1] - mdphy_1.amum[i__ - 2]) / 
		    intvr2_1.amumi[i__ - 1], abs(d__1)) < 1e-10) {
		qwrmx2 = 0.;
...
/* Computing MAX */
		d__2 = 1e-20, d__3 = vfact_1.fact5[i__ - 1] * (d__1 = (
			mdphy_1.amum[i__ - 1] - mdphy_1.amum[i__ - 2]) / 
			intvr2_1.amumi[i__ - 1], abs(d__1));
