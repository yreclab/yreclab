/* mdot.f -- translated by f2c (version 20061008).
	...
	doublereal *senvold)
{
    ...
    static char fmt_10[] = "(\002 TIMESTEP REDUCED FOR MASS LOSS - OLD,NE"
	    "W\002,1p2e12.3)";
    static char fmt_22[] = "(1p6e13.4)";
    static char fmt_20[] = "(\002MASS LOSS APPLIED - NEW M,DEL M\002,1p2e19."
	    "10)";
    ...
    static doublereal p, t, f1, f2, rb, fx, fac, beta, facr, delj, delm, dlnm,
	     rate, dmcz, dtcz, rnew, test, abund;
    static logical ldone;
    static doublereal dtmax, dtenv, dmczt, dlogr, cjtot, rinit, surfi, dttot, 
	    cjnew, ratio, wsurf, dmsun0, hstot1, hstot2, dlogen, ethnew;

    /* Fortran I/O blocks */
    static cilist io___3 = { 0, 6, 0, 0, 0 };
    static cilist io___11 = { 0, 6, 0, fmt_10, 0 };
    static cilist io___35 = { 0, 6, 0, fmt_22, 0 };
    static cilist io___39 = { 0, 6, 0, fmt_20, 0 };
...
/* MHP 8/10- CHECK FOR SCALED SOLAR WIND MASS LOSS */
    if (masschg3_1.lsolwind && rot_1.lrot) {
/* Computing 2nd power */
	d__1 = omega[*m] / masschg3_1.dmwsun;
	f1 = d__1 * d__1;
/* Computing 2nd power */
	d__1 = masschg3_1.dmwmax / masschg3_1.dmwsun;
	f2 = d__1 * d__1;
	*dmdt = masschg3_1.dmsun * min(f1,f2);
	s_wsle(&io___3);
	do_lio(&c__5, &c__1, (char *)&omega[*m], (ftnlen)sizeof(doublereal));
	do_lio(&c__5, &c__1, (char *)&(*dmdt), (ftnlen)sizeof(doublereal));
	e_wsle();
    }
/*      LMDOT = .TRUE. */
/*      DMDT = 4.0D-10 */
...
/* BEYOND THE FITTING POINT TO THE STAR. */
    if (rate > 0.) {
	dtenv = (pow_dd(&c_b7, hstot) - pow_dd(&c_b7, &hs[*m])) * .5 / rate;
...
/* OVERALL SCALE FACTOR IN R */
	t = pow_dd(&c_b7, &ht[*jenv]);
	p = pow_dd(&c_b7, &hp[*jenv]);
	d__ = pow_dd(&c_b7, &hd[*jenv]);
/* Computing 4th power */
	d__1 = t, d__1 *= d__1;
...
/*         WRITE(*,*)DLOGR,DLOGP,DLOGT */
	if (*jenv == 1) {
	    ...
	    rb = pow_dd(&c_b7, &hr[*jenv]);
	    facr = pow_dd(&c_b7, &dlogr);
	    i__1 = *m;
	    for (j = *jenv + 1; j <= i__1; ++j) {
		rinit = pow_dd(&c_b7, &hr[j]);
...
/* CONSTANT J/M */
	    ...
	    wsurf = omega[*m] * pow_dd(&c_b7, &d__1) / pow_dd(rtot, &
		    rot_1.walpcz);
...
/*  FIND TOTAL STARTING ANGULAR MOMENTUM OF C.Z. */
	...
	ratio = cjnew / cjtot;
	s_wsfe(&io___35);
	do_fio(&c__1, (char *)&delm, (ftnlen)sizeof(doublereal));
	do_fio(&c__1, (char *)&delj, (ftnlen)sizeof(doublereal));
	do_fio(&c__1, (char *)&omega[*m], (ftnlen)sizeof(doublereal));
	do_fio(&c__1, (char *)&cjnew, (ftnlen)sizeof(doublereal));
	do_fio(&c__1, (char *)&cjtot, (ftnlen)sizeof(doublereal));
	do_fio(&c__1, (char *)&dmczt, (ftnlen)sizeof(doublereal));
	e_wsfe();
	i__1 = *m;
	for (j = *jenv; j <= i__1; ++j) {
	    hjm[j] *= ratio;
...
/* LOG OF TOTAL MASS IN GRAMS (HSTOT,STOTAL) */
    *smass += delm / const_1.cmsun;
    ...
    dmsun0 = delm / const_1.cmsun;
    s_wsfe(&io___39);
    do_fio(&c__1, (char *)&(*smass), (ftnlen)sizeof(doublereal));
    do_fio(&c__1, (char *)&dmsun0, (ftnlen)sizeof(doublereal));
    e_wsfe();
    hstot1 = pow_dd(&c_b7, hstot);
    hstot2 = hstot1 + delm;
    *hstot = d_lg10(&hstot2);
    comp_1.stotal = *hstot;
